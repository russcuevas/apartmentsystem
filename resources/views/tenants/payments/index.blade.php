...
<head>
    ...
    <title>LMS Apartment - My Payments Directory</title>
    ...
    <style>
        /* Modern Glassmorphic Table & Custom Controls Styling */
        .filter-card {
        ...
        .view-month-payments-btn:active {
            transform: translateY(0);
            box-shadow: 0 2px 8px rgba(13, 148, 136, 0.2);
        }

        .btn-submit-payment svg,
        .view-month-payments-btn svg {
            transition: transform 0.2s ease;
        }

        .btn-submit-payment:hover svg,
        .view-month-payments-btn:hover svg {
...
<body>
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
                    <div>
                        ...
                        <p style="color: var(--text-muted); font-size: 0.95rem; margin-top: 4px;">
                            Track your monthly payment transactions, receipts, and verification status for Year <strong>{{ $selectedYear }}</strong>.
                        </p>
                    </div>
                    <button type="button" class="btn-submit-payment" id="openAddPaymentBtn" style="font-size: 0.9rem; padding: 10px 20px;">
                ...
                <div class="custom-table-card">
                    <div style="margin-bottom: 18px;">
                        ...
                        <p style="font-size: 0.82rem; color: var(--text-light); margin-top: 2px;">
                            Click <strong>View Payments</strong> on any month to see itemized payment entries and proof documents.
                        </p>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="room-pill">
                                                {{ $data['total_count'] }} Payment(s)
                                            </span>
                                        </td>
                                        <td>
                                            <strong style="color: {{ $data['total_amount'] > 0 ? '#166534' : '#64748b' }}; font-size: 0.95rem;">
                                                ₱{{ number_format($data['total_amount'], 2) }}
                                            </strong>
                                        </td>
                                        <td>
                                            <button type="button"
                                                class="view-month-payments-btn"
            ...
            <div class="modal-header">
                <div>
                    ...
                    <p style="font-size: 0.82rem; color: #64748b; margin-top: 2px;">
                        Tenant: <strong>{{ $tenant->fullname }}</strong> | Room: <strong>{{ $tenant->rentInformation->room ?? 'N/A' }}</strong> ({{ $tenant->location->location_name ?? 'N/A' }})
                    </p>
                </div>
                <button type="button" class="modal-close-btn" id="closeAddPaymentBtn">&times;</button>
            </div>
            <form action="{{ route('tenant.payments.store') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; overflow: hidden; max-height: calc(85vh - 70px);">
                @csrf
                <input type="hidden" name="tenant_id" value="{{ $tenant->id }}">

                <div class="modal-body" style="padding: 24px; overflow-y: auto; max-height: calc(85vh - 140px); -webkit-overflow-scrolling: touch;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 20px;">
                        <div>
                            ...
                            <select name="category" id="paymentCategorySelect" class="filter-select" style="width: 100%; height: 42px;" required>
                        ...
                        <div>
                            ...
                            <select name="billing_month" class="filter-select" style="width: 100%; height: 42px;" required>
                    ...
                    <div id="proofOfBillingGroup" style="display: none; background: #f0f9ff; border: 1.5px dashed #0284c7; padding: 18px; border-radius: 12px; margin-bottom: 20px;">
                        <div style="font-size: 0.88rem; font-weight: 800; color: #0369a1; margin-bottom: 12px; display: flex; align-items: center; gap: 6px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 16px;">
                            <div>
                                ...
                                <input type="file" name="proof_of_billing" id="proofOfBillingInput" accept="image/*,application/pdf" class="filter-select" style="width: 100%;">
                                <div id="proofOfBillingPreview" style="margin-top: 6px;"></div>
                            </div>

                            <div id="utilityAmountContainer">
                                <label id="utilityAmountLabel" style="display: block; font-size: 0.84rem; font-weight: 700; color: #334155; margin-bottom: 6px;">
                                    Utility Bill Total Amount (₱) <span style="color: #ef4444;">*</span>
                                </label>
                                <input type="number" step="0.01" min="0" id="utilityAmountInput" name="electricity_amount" class="filter-select" style="width: 100%; height: 42px;" placeholder="Type total amount from billing receipt...">
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 20px;">
                        <div>
                            ...
                            <select name="type" id="paymentTypeSelect" class="filter-select" style="width: 100%; height: 42px;" required>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0.01" name="amount" class="filter-select" style="width: 100%; height: 42px;" placeholder="0.00" required>
                        </div>
                    </div>

                    <!-- 4. Handed To / Received By Name (Shown only when CASH is selected) -->
                    <div id="getFullnameGroup" style="display: none; background: #fff7ed; border: 1px solid #ffedd5; padding: 16px; border-radius: 12px; margin-bottom: 20px;">
                        <label style="display: block; font-size: 0.84rem; font-weight: 700; color: #c2410c; margin-bottom: 6px;">
                            Handed / Received By (Full Name) <span style="color: #ef4444;">*</span>
                        </label>
                        <input type="text" name="get_fullname" id="getFullnameInput" class="filter-select" style="width: 100%; height: 42px;" placeholder="Type the full name of admin/person who received cash...">
                    </div>

                    <!-- 5. Upload Payment Proof / Receipt Card -->
                    <div style="background: #f8fafc; border: 1.5px dashed #cbd5e1; padding: 18px; border-radius: 12px;">
                        <label style="display: block; font-size: 0.84rem; font-weight: 700; color: #334155; margin-bottom: 6px;">
                            Upload Payment Receipt / Transfer Proof <span style="color: #ef4444;">*</span>
                        </label>
                        <input type="file" name="payment_proof" id="paymentProofInput" accept="image/*,application/pdf" class="filter-select" style="width: 100%;">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-submit-payment" style="margin-left: 10px;">✓ Submit Payment</button>
                </div>
            </form>
        </div>
    </div>

    <!-- MODAL: MONTH PAYMENTS LIST MODAL -->
    <div class="modal-overlay" id="monthPaymentsModal">
        <div class="modal-container-lg">
            <div class="modal-header">
                <div>
                    <h3 class="modal-title" id="monthPaymentsModalTitle">Payment Details</h3>
                    <p style="font-size: 0.82rem; color: #64748b; margin-top: 2px;">
                        Detailed log of payments submitted for the selected month.
                    </p>
                </div>
    ...
    <div class="modal-overlay" id="imageViewerModal" style="z-index: 10050;">
        <div class="modal-container-lg" style="max-width: 800px; background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.1); color: #fff;">
            <div class="modal-header" style="background: transparent; border-bottom: 1px solid rgba(255,255,255,0.1);">
                ...
                <button type="button" class="modal-close-btn" id="closeImageViewerBtn" style="color: #ffffff;">&times;</button>
            </div>
            <div class="modal-body" style="display: flex; align-items: center; justify-content: center; padding: 20px; text-align: center; overflow: hidden;">
                <img src="" id="fullImageViewerSrc" style="max-width: 100%; max-height: 75vh; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); object-fit: contain;">
            </div>
    ...
    <!-- Notyf JS -->
    <script src="https://cdn.jsdelivr.net/npm/notyf@3/notyf.min.css"></script>
    <script src="https://cdn.jsdelivr.net/npm/notyf@3/notyf.min.js"></script>

    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Dropdowns & Mobile Sidebar Setup
            ...
            if (openAddPaymentBtn) openAddPaymentBtn.addEventListener('click', () => openModal(addTenantPaymentModal));
            if (closeAddPaymentBtn) closeAddPaymentBtn.addEventListener('click', () => closeModal(addTenantPaymentModal));
            if (cancelAddPaymentBtn) cancelAddPaymentBtn.addEventListener('click', () => closeModal(addTenantPaymentModal));

            if (closeMonthPaymentsBtn) closeMonthPaymentsBtn.addEventListener('click', () => closeModal(monthPaymentsModal));
            if (closeMonthPaymentsFooterBtn) closeMonthPaymentsFooterBtn.addEventListener('click', () => closeModal(monthPaymentsModal));

            if (closeImageViewerBtn) closeImageViewerBtn.addEventListener('click', () => closeModal(imageViewerModal));
            if (closeImageViewerFooterBtn) closeImageViewerFooterBtn.addEventListener('click', () => closeModal(imageViewerModal));
            ...
            // Click listener for clickable preview images & view proof links
            ...
            $(document).on('click', '.view-proof-img-link', function(e) {
                ...
                const title = $(this).data('title') || 'Proof Document';
                $('#fullImageViewerSrc').attr('src', src);
            ...
            // Category Selection Change Handler (Monthly Rental vs Electricity vs Water)
            $('#paymentCategorySelect').on('change', function() {
                const cat = $(this).val();
                if (cat === 'Electricity' || cat === 'Water') {
                    ...
                    if (cat === 'Electricity') {
                        $('#utilityAmountLabel').html('Electricity Bill Total Amount (₱) <span style="color: #ef4444;">*</span>');
                        $('#utilityAmountInput').attr('name', 'electricity_amount').prop('required', true);
                    } else {
                        $('#utilityAmountLabel').html('Water Bill Total Amount (₱) <span style="color: #ef4444;">*</span>');
                        $('#utilityAmountInput').attr('name', 'water_amount').prop('required', true);
                    }
                } else {
            ...
            $(document).on('click', '.view-month-payments-btn', function() {
                ...
                $('#monthPaymentsModalTitle').text(`Payment Records for ${month} ${year}`);
                ...
                if (Array.isArray(payments) && payments.length > 0) {
                    payments.forEach(function(p) {
                        ...
                        let proofHtml = `<span style="color: #94a3b8; font-style: italic;">No proof attached</span>`;
                        if (p.payment_proof && p.payment_proof !== '') {
                            const isImg = /\.(jpg|jpeg|png|webp|gif)$/i.test(p.payment_proof);
                            if (isImg) {
                                proofHtml = `<a href="javascript:void(0)" class="view-proof-img-link" data-src="${p.payment_proof}" data-title="Payment Proof #${p.id}" style="color: var(--primary); font-weight: 700; text-decoration: underline;">View Proof</a>`;
                            } else {
                                proofHtml = `<a href="${p.payment_proof}" target="_blank" style="color: var(--primary); font-weight: 700; text-decoration: underline;">View Proof (PDF)</a>`;
                            }
                        }

                        const receiverName = p.receiver ? p.receiver.fullname : (status === 'Accepted' ? 'Admin' : 'Pending');

                        const rowHtml = `
                            <tr>
                                <td>
                                    <div>
                                        <strong style="font-size: 0.88rem; color: #0f172a;">${dateStr}</strong>
                ...
                monthPaymentsDataTable = $('#monthPaymentsTable').DataTable({
                    ...
                    language: {
                        ...
                        emptyTable: `No payment transactions submitted for ${month} ${year}.`,
                        zeroRecords: `No payment transactions submitted for ${month} ${year}.`
                    }
                });
